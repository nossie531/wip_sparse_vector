//! Provider of [`SparseVecSample`].

use crate::for_test::builders::*;
use sparse_vector::prelude::*;
use std::iter::{self, repeat_with};

const RANDOM_TEST_SIZE: usize = 64;

/// Samples for [`SparseVec`].
pub struct SparseVecSample();

impl SparseVecSample {
    pub fn default() -> SparseVec<i32> {
        SparseVec::default()
    }

    pub fn single() -> SparseVec<i32> {
        SparseVecBuilder::new().set_len(1).set_nnp(1).build()
    }

    pub fn normal() -> SparseVec<i32> {
        SparseVecBuilder::new().build()
    }

    pub fn normal_floats() -> SparseVec<f32> {
        SparseVecBuilder::new().build_floats()
    }

    pub fn all_padding() -> SparseVec<i32> {
        SparseVecBuilder::new().set_nnp(0).build()
    }

    pub fn random_trivals(seed: u64) -> SparseVec<i32> {
        let value_range = -1..=1;
        let padding = -1 + (seed / 3) as i32;
        SparseVecBuilder::new()
            .set_seed(seed)
            .set_padding(padding)
            .set_value_range(value_range)
            .build()
    }

    pub fn pairs() -> impl Iterator<Item = [SparseVec<i32>; 2]> {
        const CUSTOMS: [fn() -> [SparseVec<i32>; 2]; 10] = [
            default_eq_default,
            single_eq_single,
            normal_eq_normal,
            normal_eq_diff_padding_v1,
            normal_eq_diff_padding_v2,
            padding_lt_normal,
            value1_lt_value2,
            normal_lt_extra_padding,
            normal_lt_extra_value,
            normal_lt_diff_padding,
        ];

        let customs = custom_pairs();
        let randoms = random_trivals_pairs();
        return customs.chain(randoms);

        fn custom_pairs() -> impl Iterator<Item = [SparseVec<i32>; 2]> {
            let mut customs = CUSTOMS.iter();
            iter::from_fn(move || Some(customs.next()?()))
        }

        fn random_trivals_pairs() -> impl Iterator<Item = [SparseVec<i32>; 2]> {
            let mut seed = 0;
            repeat_with(move || {
                let ret = random_trivals_pair(seed);
                seed += 1;
                ret
            })
            .take(RANDOM_TEST_SIZE)
        }

        fn default_eq_default() -> [SparseVec<i32>; 2] {
            [SparseVecSample::default(), SparseVecSample::default()]
        }

        fn single_eq_single() -> [SparseVec<i32>; 2] {
            [SparseVecSample::single(), SparseVecSample::single()]
        }

        fn normal_eq_normal() -> [SparseVec<i32>; 2] {
            [SparseVecSample::normal(), SparseVecSample::normal()]
        }

        fn normal_eq_diff_padding_v1() -> [SparseVec<i32>; 2] {
            let mut target_x = SparseVec::<i32>::with_padding(0, 0);
            let mut target_y = SparseVec::<i32>::with_padding(0, 1);
            target_x.extend([0, 1, 0]);
            target_y.extend([0, 1, 0]);
            [target_x, target_y]
        }

        fn normal_eq_diff_padding_v2() -> [SparseVec<i32>; 2] {
            let mut target_x = SparseVec::<i32>::with_padding(0, 0);
            let mut target_y = SparseVec::<i32>::with_padding(0, 1);
            target_x.extend([0, 0, 1]);
            target_y.extend([0, 0, 1]);
            [target_x, target_y]
        }

        fn padding_lt_normal() -> [SparseVec<i32>; 2] {
            let builder = SparseVecBuilder::new();
            let target_x = builder.build();
            let mut target_y = builder.build();
            let index = builder.some_pad_indexs(1)[0];
            *target_y.edit(index) += 1;
            [target_x, target_y]
        }

        fn value1_lt_value2() -> [SparseVec<i32>; 2] {
            let builder = SparseVecBuilder::new();
            let target_x = builder.build();
            let mut target_y = builder.build();
            let index = builder.some_npad_indexs(1)[0];
            *target_y.edit(index) += 1;
            [target_x, target_y]
        }

        fn normal_lt_extra_padding() -> [SparseVec<i32>; 2] {
            let target_x = SparseVecSample::normal();
            let mut target_y = SparseVecSample::normal();
            target_y.set_len(target_y.len() + 1);
            [target_x, target_y]
        }

        fn normal_lt_extra_value() -> [SparseVec<i32>; 2] {
            let target_x = SparseVecSample::normal();
            let mut target_y = SparseVecSample::normal();
            target_y.extend([42]);
            [target_x, target_y]
        }

        fn normal_lt_diff_padding() -> [SparseVec<i32>; 2] {
            let mut target_x = SparseVec::<i32>::with_padding(0, 0);
            let mut target_y = SparseVec::<i32>::with_padding(0, 1);
            target_x.extend([0, 0, 0]);
            target_y.extend([1, 1, 0]);
            [target_x, target_y]
        }

        fn random_trivals_pair(seed: u64) -> [SparseVec<i32>; 2] {
            let trivals_x = SparseVecSample::random_trivals(seed);
            let trivals_y = SparseVecSample::random_trivals(seed + 1);
            [trivals_x, trivals_y]
        }
    }
}